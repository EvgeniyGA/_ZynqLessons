...
int main(){
	...
	for(Index = 0; Index < myVDMA.MaxNumFrames; Index++) {	//this MaxNumFrames defined in xparameters.h
		ReadCfg.FrameStoreStartAddr[Index] = Addr;			//as XPAR_XAXIVDMA_NUM_INSTANCES or XPAR_AXI_VDMA_0_NUM_FSTORES
		Addr +=  FrameSize;
	}
...
static void ReadCallBack(void *CallbackRef, u32 Mask)
{
	...
	//it unconnect this, will blink black and white
	//and buffer can be deleted
	/*if(i==0){
	...
	for(int i=0;i<VSize;i++){
		for(int j=0;j<HSize;j++){
			if(i<vOffset || i >= vOffset+imageVSize){
				...
				Buffer[(i*HSize*3)+j*3]     = (*imagePointer/16)*4;
			    Buffer[(i*HSize*3)+(j*3)+1] = (*imagePointer/16)*4;
			    Buffer[(i*HSize*3)+(j*3)+2] = (*imagePointer/16)*4;
			    imagePointer++;
			}
		}
	}
